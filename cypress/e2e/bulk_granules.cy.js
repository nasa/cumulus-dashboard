describe('Dashboard Bulk Granules', () => {
  describe('When logged in', () => {
    ...
    it('handles successful bulk granule reingest request with a s3 granuleId file', () => {
      const asyncOperationId = Math.random().toString(36).substring(2, 15);
      ...
      cy.get('.bulk_granules--reingest')
        .within(() => {
          cy.get('.ace_editor[id^="run-bulk-granule"]', { timeout: 10000 });
          cy.editJsonTextarea({ data: { s3GranuleIdInputFile: 's3://localbucket/localrun/reconciliation-reports/GranuleList100220.csv' }, update: true });
