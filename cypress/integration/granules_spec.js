...
describe('Dashboard Granules Page', () => {
  ...
  describe('When logged in', () => {
    ...
    it('should display all granules in table with correct columns', () => {
      ...
      // checks that duplicate producerIds can exist in the table
      cy.get('@granulesListFixture').its('results').then((results) => {
        const producerGranuleIds = results.map((g) => g.producerGranuleId);
    ...
    it('should show number of selected results in table', () => {
      ...
      cy.contains('.table__header', '(13 selected)');
    });
