...
describe('Dashboard Granules Page', () => {
  ...
  describe('When logged in', () => {
    ...
    it('Should update results when toggling Search By Prefix', () => {
      const prefix = 'MOD09GQ.A241';
      const infix = 'A153';
      cy.visit('/granules');
      cy.get('#chk_isprefixsearch').should('be.checked');
      cy.get('.search').as('search');
      cy.get('@search').click().type(prefix);
      cy.get('.table .tbody .tr').should('have.length', 1);
      cy.get('@search').click().type(infix);
      cy.get('.table .tbody .tr').should('have.length', 0);
      cy.get('#chk_isprefixsearch').click().should('not.be.checked');
      cy.get('.table .tbody .tr').should('have.length.at.least', 1);
      cy.get('.table .tbody .tr').eq(0).children('.td').eq(2)
        .contains(infix);
    });

    it('Should have Search by prefix input in granule status list view and update results when toggling Search By Prefix', () => {
      const prefix = 'test_';
      const infix = 'A201';
    ...
    it('Should update the table when the Results Per Page and Page dropdowns are changed.', () => {
      ...
      cy.get('@limit-input').should('be.visible').click({ force: true }).type('{backspace}{backspace}10{enter}');
    ...
    it('Should reingest a granule from granule detail page.', () => {
      ...
      cy.get('.modal-body .form__dropdown .dropdown__element .dropdown', { timeout: 10000 })
        .should('contain.text', 'IngestAndPublishGranule');
    ...
    it('Should reingest a granule and redirect to the granule detail page.', () => {
      ...
      cy.intercept('GET', `/granules/${granuleId}*`).as('getGranule');
      cy.visit('/granules');
      cy.get(`[data-value="${granuleId}"] > .td >input[type="checkbox"]`).click(clickOptions);
    ...
    it('should navigate to executions-list when clicking on the executions-list link for a granule', () => {
      ...
      cy.get('.tr:nth-child(1) > .td:nth-child(6) > a').click(clickOptions);
    ...
    it('should navigate to to executions-list when executions-list link is clicked on granule detail page', () => {
      ...
      cy.get('.tr:nth-child(1) > .td:nth-child(3) > div > span > a').click({ force: true });
