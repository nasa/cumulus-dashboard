...
export const kibanaTEALambdaErrorsLink = (cumulusInstanceMeta, datepicker) => {
  ...
  const { stackName } = cumulusInstanceMeta;
  const timeInterval = formatKibanaDate(datepicker);
  return `${_config.kibanaRoot}/app/discover#/?_g=(refreshInterval:(pause:!t,value:0),time:(${timeInterval}))&_a=(columns:!(_source),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,index:${stackName},key:logGroup,negate:!f,params:(query:%2Faws%2Flambda%2F${stackName}-thin-egress-app-EgressLambda),type:phrase),query:(match:(logGroup:(query:%2Faws%2Flambda%2F${stackName}-thin-egress-app-EgressLambda,type:phrase))))),index:${stackName},interval:auto,query:(language:lucene,query:'response_status:failure'),sort:!('@timestamp',desc))`;
};
...
export const kibanaGatewayExecutionErrorsLink = (cumulusInstanceMeta, datepicker) => {
  ...
  const { stackName } = cumulusInstanceMeta;
  const timeInterval = formatKibanaDate(datepicker);
  return `${_config.kibanaRoot}/app/discover#/?_g=(refreshInterval:(pause:!t,value:0),time:(${timeInterval}))&_a=(columns:!(message),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,index:${stackName},key:logGroup,negate:!f,params:(query:'%22API%5C-Gateway%5C-Execution*%22'),type:phrase),query:(match:(logGroup:(query:'%22API%5C-Gateway%5C-Execution*%22',type:phrase))))),index:${stackName},interval:auto,query:(language:lucene,query:'%2B%22Method%20completed%20with%20status:%22%20%2B(4%3F%3F%205%3F%3F)'),sort:!('@timestamp',desc))`;
};
...
export const kibanaGatewayExecutionSuccessesLink = (cumulusInstanceMeta, datepicker) => {
  ...
  const { stackName } = cumulusInstanceMeta;
  const timeInterval = formatKibanaDate(datepicker);
  const link = `${_config.kibanaRoot}/app/discover#/?_g=(refreshInterval:(pause:!t,value:0),time:(${timeInterval}))&_a=(columns:!(message),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,index:${stackName},key:logGroup,negate:!f,params:(query:'%22API%5C-Gateway%5C-Execution*%22'),type:phrase),query:(match:(logGroup:(query:'%22API%5C-Gateway%5C-Execution*%22',type:phrase))))),index:${stackName},interval:auto,query:(language:lucene,query:'%2B%22Method%20completed%20with%20status:%22%20%2B(2%3F%3F%203%3F%3F)'),sort:!('@timestamp',desc))`;
  return link;
};
