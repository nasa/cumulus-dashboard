...
const kibanaConfigured = (cumulusInstanceMeta) => !!cumulusInstanceMeta &&
    !!cumulusInstanceMeta.stackName &&
    !!_config.kibanaCloudwatchUUID &&
    !!_config.kibanaDistributionUUID &&
    !!_config.kibanaGranulesUUID &&
    !!_config.kibanaRoot &&
    !!_config.kibanaSecurityTenant;
...
export const kibanaS3AccessErrorsLink = (cumulusInstanceMeta, datepicker) => {
  ...
  return `${_config.kibanaRoot}/app/discover?security_tenant=${_config.kibanaSecurityTenant}#/?_g=(refreshInterval:(pause:!t,value:0),time:(${timeInterval}))&_a=(columns:!(key,response),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,index:'${_config.kibanaDistributionUUID}',key:operation,negate:!f,params:(query:REST.GET.OBJECT),type:phrase),query:(match_phrase:(operation:REST.GET.OBJECT)))),index:'${_config.kibanaDistributionUUID}',interval:auto,query:(language:lucene,query:'NOT%20response:200'),sort:!(!('@timestamp',desc)))`;
};
...
export const kibanaTEALambdaErrorsLink = (cumulusInstanceMeta, datepicker) => {
  ...
  const timeInterval = formatKibanaDate(datepicker);
  return `${_config.kibanaRoot}/app/discover?security_tenant=${_config.kibanaSecurityTenant}#/?_g=(refreshInterval:(pause:!t,value:0),time:(${timeInterval}))&_a=(columns:!(s3,code,response_status,redirect_URL),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,index:${_config.kibanaCloudwatchUUID},key:logGroup,negate:!f,params:(query:%2Faws%2Flambda%2F${stackName}-thin-egress-app-EgressLambda),type:phrase),query:(match_phrase:(logGroup:%2Faws%2Flambda%2F${stackName}-thin-egress-app-EgressLambda)))),index:${_config.kibanaCloudwatchUUID},interval:auto,query:(language:lucene,query:'response_status:failure'),sort:!(!('@timestamp',desc)))`;
};
...
export const kibanaGatewayExecutionErrorsLink = (cumulusInstanceMeta, datepicker) => {
  if (!kibanaConfigured(cumulusInstanceMeta)) return '';
  const timeInterval = formatKibanaDate(datepicker);
  return `${_config.kibanaRoot}/app/discover?security_tenant=${_config.kibanaSecurityTenant}#/?_g=(refreshInterval:(pause:!t,value:0),time:(${timeInterval}))&_a=(columns:!(message),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,index:${_config.kibanaCloudwatchUUID},key:logGroup,negate:!f,params:(query:'API-Gateway-Execution-.*'),type:phrase),query:(match_phrase:(logGroup:'API-Gateway-Execution-.*')))),index:${_config.kibanaCloudwatchUUID},interval:auto,query:(language:lucene,query:'%2B%22Method%20completed%20with%20status:%22%20%2B(4%3F%3F%205%3F%3F)'),sort:!(!('@timestamp',desc)))`;
};
...
export const kibanaAllLogsLink = (cumulusInstanceMeta, datepicker) => {
  if (!kibanaConfigured(cumulusInstanceMeta)) return '';
  const timeInterval = formatKibanaDate(datepicker);
  return `${_config.kibanaRoot}/app/discover?security_tenant=${_config.kibanaSecurityTenant}#/?_g=(refreshInterval:(pause:!t,value:0),time:(${timeInterval}))&_a=(columns:!(_source),filters:!(),index:${_config.kibanaCloudwatchUUID},interval:auto,query:(language:lucene,query:''),sort:!(!('@timestamp',desc)))`;
};

export const kibanaGranuleErrorsLink = (cumulusInstanceMeta, datepicker) => {
  if (!kibanaConfigured(cumulusInstanceMeta)) return '';
  const timeInterval = formatKibanaDate(datepicker);
  return `${_config.kibanaRoot}/app/discover?security_tenant=${_config.kibanaSecurityTenant}#/?_g=(refreshInterval:(pause:!t,value:0),time:(${timeInterval}))&_a=(columns:!(_source),filters:!(),index:'${_config.kibanaGranulesUUID}',interval:auto,query:(language:lucene,query:'status:%20failed'),sort:!(!('@timestamp',desc)))`;
};
