...
test('CUMULUS-799 Home page contains distribution error report', async (t) => {
  ...
  config.kibanaRoot = '/fake/local/link/';
  const home = shallow(
    <Home
      dispatch={dispatch}
      dist={dist}
  ...
  t.is(errorMetrics.at(1).find('Link').dive().text(), `${tally(dist.apiGateway.execution.errors)} Gateway Execution Errors`);
  t.is(errorMetrics.at(2).find('Link').dive().text(), `${tally(dist.apiGateway.access.errors)} Gateway Access Errors`);
  ...
  t.is(successMetrics.at(1).find('Link').dive().text(), `${tally(dist.teaLambda.successes)} TEA Lambda Successes`);
  t.is(successMetrics.at(2).find('Link').dive().text(), `${tally(dist.apiLambda.successes)} Distribution API Lambda Successes`);
  t.is(successMetrics.at(3).find('Link').dive().text(), `${tally(dist.apiGateway.execution.successes)} Gateway Execution Successes`);
  t.is(successMetrics.at(4).find('Link').dive().text(), `${tally(dist.apiGateway.access.successes)} Gateway Access Successes`);
});
